...
- [ ] 1.1 Add `HeadCompensator` to `Sources/GazeKit/`: init from calibration-capture head poses/spans (baseline = unweighted per-axis means over valid finite samples; span baseline absent if none), delta computation against the baseline, and a zero-initialized bounded additive linear correction (`corrected = mapped + clamp(J·d)`, clamp = Euclidean-norm radial scaling)
- [ ] 1.2 Add the internal stability window (I-DT style dispersion + min-duration) and gated online learning: per-sample regularized update regressing the **negative** anchor deviation against the relative head delta (so `J` cancels head-induced motion), with forgetting factor and per-step cap; no updates during saccades, below the confidence threshold, or outside the hard learning domain — the user-facing alarm never gates learning (recoverability)
...
- [ ] 1.5 Unit tests mapping 1:1 to the spec scenarios (synthetic streams only): zero-knowledge identity, unlearned alarm matches the legacy threshold exactly, learned correction reduces head-correlated error, radial cap clamping, correction continuity under smoothly-varying head deltas, saccade-only streams learn nothing, no learning outside the hard domain, an alarmed dwell still teaches and the alarm recovers without recalibration, alarm on extreme drift / span breach / compensated-drift-no-alarm, invalid baseline samples excluded, reset on recalibration, full path driven without any platform framework
